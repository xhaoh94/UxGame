...
namespace Ux.UI
{
	...
	public partial class MainView : UIView
	{
		...
		{
			btnMultiple.AddClick(_OnBtnMultipleClick);
			btnNoneWindow.AddClick(_OnBtnNoneWindowClick);
			btnNoneView.AddClick(_OnBtnNoneViewClick);
			btnSingle.AddClick(_OnBtnSingleClick);
			btnTest.AddClick(_OnBtnTestClick);
			btnBack.AddClick(_OnBtnBackClick);
			btnDouble.AddMultipleClick(_OnBtnDoubleMultipleClick, 2, 0.2f);
			btnLongClick.AddLongPress(-1f, _OnBtnLongClickLongPress, 0.2f, 0, 50);
		}
