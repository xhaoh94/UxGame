...
public class TerrainGrid : BaseGrid
{
    ...
    readonly FogOfWarComponent _fogOfWar;
    public TerrainGrid(FogOfWarComponent fogOfWar, int w, int h) : base(w, h)
    {
        _fogOfWar = fogOfWar;
        m_width = w;
        m_height = h;
    ...
    {
        Vector2Int tilePos = _fogOfWar.WorldPosToTilePos(worldPos);
        GetData(tilePos.x, tilePos.y, out altitude, out grassId);
    }
}