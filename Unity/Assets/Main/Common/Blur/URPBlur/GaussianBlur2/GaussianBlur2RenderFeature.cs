...
public class GaussianBlur2RenderFeature : ScriptableRendererFeature
{
    ...
    public override void Create()         //固定写法-override void Create是 父类ScriptableRendererFeature的内容
    {
        ...
        _blurPass = new GaussianBlurPass2(renderPassEvent, shader);      // 创建pass-(在RenderFeature面板可以看到pass的输入参数)
    }
    ...
    public class GaussianBlurPass2 : ScriptableRenderPass
    {
        ...
        public GaussianBlurPass2(RenderPassEvent evt, Shader shader)        // 构造函数输入 RenderPassEvent-渲染事件位置, Pass所使用的Shader;这2个参数会暴露在面板中
        {
            this.renderPassEvent = evt;              //固定写法----设置渲染事件位置 - renderPassEvent             
            if (shader)
            {
                this._mat = CoreUtils.CreateEngineMaterial(shader);       //新建材质
            }
        }
