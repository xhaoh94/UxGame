    ...
    public class PackageCompareWindow : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (string.IsNullOrEmpty(_manifestPath1) == false && string.IsNullOrEmpty(_manifestPath2) == false)
            {
                ...
                {
                    CompareManifest(_changeList, _newList);
                }
            }
        ...
        private void CompareManifest(List<PackageBundle> changeList, List<PackageBundle> newList)
        {
            changeList.Clear();
            newList.Clear();
            ...
            var packageManifest1 = ManifestTools.DeserializeFromBinary(bytesData1);

            // 加载补丁清单1
            byte[] bytesData2 = FileUtility.ReadAllBytes(_manifestPath2);
            var packageManifest2 = ManifestTools.DeserializeFromBinary(bytesData2);

            // 拷贝文件列表
            foreach (var bundle2 in packageManifest2.BundleList)
            {
                if (packageManifest1.TryGetPackageBundle(bundle2.BundleName, out PackageBundle bundle1))
                {
