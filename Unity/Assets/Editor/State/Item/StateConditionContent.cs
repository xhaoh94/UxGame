...
public class StateConditionContent : TemplateContainer
{
    ...
    public void SetData(StateCondition enterCondition)
    {
    ...
    public void UpdateView()
    {
        ...
        switch (stateEnter.Type)
        {
            case StateEnterConditionType.State:
                item = new StateItem();
                break;
            case StateEnterConditionType.TempBoolVar:
                item = new TempBoolVarItem();
                break;
            case StateEnterConditionType.Action_Keyboard:
                item = new ActionKeyboardItem();
                break;
            case StateEnterConditionType.Action_Input:
                item = new ActionInputItem();
                break;
        }
        if (item != null)
        {
            item.SetData(stateEnter);
            content.Add(item);
        }
    }
}