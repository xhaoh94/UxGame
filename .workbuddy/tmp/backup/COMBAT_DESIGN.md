...
| 预测与回滚 | `CombatActionRunner.cs:275` `Confirm` / `:301` `Reject`；`CombatController.cs:149` `CaptureSnapshot` | 客户端预测 + 服务器纠偏的骨架已完备 |
| 输入命令 | `Manager/Combat/Asset/CombatCommand.cs` | 逐帧命令队列，本地 / 网络 / 录像共用 |
| 表现桥接 | `Manager/Combat/Runtime/Presentation/CombatTimelinePlayer.cs:36` `CombatTimelineResolver.Resolve` | Life > Control > Locomotion 优先级选基础层；Life/Control 命中时独占并停掉 Action 层 |
